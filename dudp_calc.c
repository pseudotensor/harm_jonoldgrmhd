...
int dudp_calc(FTYPE *pr, struct of_state *q, struct of_geom *geom,
	      FTYPE **alpha)
{
  FTYPE w, bsq;
  ...
  for (i = 0; i < NDIM; i++)
    for (j = 1; j < NDIM; j++)
      for (k = 0; k < NDIM; k++)
	ducov_dv[i][j] += geom->gcov[i][k] * ducon_dv[k][j];
  ...
  for (i = 0; i < NDIM; i++)
    for (j = 1; j < NDIM; j++)
      for (k = 0; k < NDIM; k++)
	dbcov_dv[i][j] += geom->gcov[i][k] * dbcon_dv[k][j];
  ...
  /* N.B.: all the conserved variables contain a factor of
     \sqrt{det(g_{\mu\nu})} */
  for (j = 1; j <= 5; j++)
    for (k = 1; k <= 5; k++)
      alpha[j][k] *= geom->g;
