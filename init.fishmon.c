...
int init()
{
  ...
  */  
  ...
  DTr = 512;			/* restart file period in steps */
  ...
  ZSLOOP(0, N1 - 1, 0, N2 - 1) {
    ...
    if (lnh < 0. || r < rin) {
      ...
      /* these valuables are demonstrably physical for all values of a
         and r */
      ...
      /*
      if(defcoord==1){
	ur = -1. / (r * r) / log(Rout / Rin);
      }
      else{
	ur = -1. / (r * r);
	//ur = -2.0 / (r * r); // for R0=.9 a=.99
	//ur = -3.0 / (r * r); // for R0=1.0 a=.99
	//	ur = -1.0 / ((r-R0) * (r-R0)); // for R0=1.0 a=.99
      }
      uh = 0.;
      up = 0.;
      */
